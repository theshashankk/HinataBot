...
from Hinatabot import DRAGONS as SUDO_USERS
from Hinatabot import pbot
from Hinatabot.modules.sql import forceSubscribe_sql as sql
            ...
            if chat_member.restricted_by.id == (client.get_me()).id:
                try:
                    ...
                    client.answer_callback_query(
                        cb.id,
                        text=f"hey I wait first join @{channel} and Then press the 'UnMute Me' button again.",
                        show_alert=True,
                    )
            else:
                client.answer_callback_query(
                    cb.id,
                    text="❗ Admin has muted you for some other reason. lol xD 😂",
                    show_alert=True,
                )
        else:
            if (
