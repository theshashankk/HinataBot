...
import Hinatabot.modules.sql.feds_sql as sql
from Hinatabot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
...
from Hinatabot.modules.helper_funcs.chat_status import is_user_admin
from Hinatabot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from Hinatabot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @CoffinXsupport if the problem persist."
            )
            return
