...
import hinataBot.modules.sql.notes_sql as sql
from hinataBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from hinataBot.__main__ import DATA_IMPORT
from hinataBot.modules.helper_funcs.chat_status import user_admin
from hinataBot.modules.helper_funcs.alternate import typing_action

# from hinataBot.modules.rules import get_rules
import hinataBot.modules.sql.rules_sql as rulessql

# from hinataBot.modules.sql import warns_sql as warnssql
import hinataBot.modules.sql.blacklist_sql as blacklistsql
from hinataBot.modules.sql import disable_sql as disabledsql

# from hinataBot.modules.sql import cust_filters_sql as filtersql
# import hinataBot.modules.sql.welcome_sql as welcsql
import hinataBot.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    with open("hinataBot{}.backup".format(chat_id), "w") as f:
        f.write(str(baccinfo))
    context.bot.sendChatAction(current_chat_id, "upload_document")
    hinataBotl = time.strftime("%H:%M:%S - %d/%m/%Y", time.localtime(time.time()))
    try:
        context.bot.sendMessage(
            JOIN_LOGGER,
            "*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`".format(
                chat.title, chat_id, hinataBotl
            ),
            parse_mode=ParseMode.MARKDOWN,
        )
    except BadRequest:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("hinataBot{}.backup".format(chat_id), "rb"),
        caption="💾*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `hinataBot-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, hinataBotl
        ),
        timeout=360,
