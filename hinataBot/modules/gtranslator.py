...
def totranslate(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if source_lang is None:
            ...
            trans_str = trl.translate(text, lang_hinataBott=dest_lang)
            return message.reply_text(
                f"Translated from `{detection[0]}` to `{dest_lang}`:\n`{trans_str}`",
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
            trans_str = trl.translate(text, lang_hinataBott=dest_lang, lang_src=source_lang)
            message.reply_text(
...
sm,sn,so,sq,sr,st,su,sv,sw,ta,te,hinataBot,th,tl,tr,uk,ur,uz,
