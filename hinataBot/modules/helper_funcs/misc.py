...
from hinataBot import LOAD, NO_LOAD
...
def paginate_modules(page_n: int, module_dict: Dict, prefix, chat=None) -> List:
    ...
    if len(pairs) > 7:
        pairs = pairs[modulo_page * 7:7 * (modulo_page + 1)] + [
            (EqInlineKeyboardButton("👈🏻 𝐋𝐄𝐅𝐓", callback_data="{}_prev({})".format(prefix, modulo_page)),
                EqInlineKeyboardButton("❖ 𝐁𝐀𝐂𝐊 ❖", callback_data="hinata_back"),
             EqInlineKeyboardButton("𝐑𝐈𝐆𝐇𝐓 👉🏻", callback_data="{}_next({})".format(prefix, modulo_page)))]
