...
from hinataBot import BOT_ID
from hinataBot.conf import get_int_key, get_str_key

# from hinataBot.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from hinataBot.pyrogramee.telethonbasics import is_admin
from hinataBot.events import register
from hinataBot import MONGO_DB_URI 
from pymongo import MongoClient
from hinataBot.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from hinataBot import telethn as tbot
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](hinataBot://user?id={hh}) your message contain NSFW content.. So, hinata deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](hinataBot://user?id={hh})  \n\n`⚔️Automatic Detections Powered By hinataAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, hinata deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By hinataAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
            ...
            if event.chat_id == c["id"]:
                if better_profanity.profanity.contains_profanity(msg):
                    ...
                    if sender.username is None:
                        st = sender.first_name
                        hh = sender.id
                        final = f"[{st}](hinataBot://user?id={hh}) **{msg}** is detected as a slang word and your message has been deleted"
                    else:
        ...
        if event.text:
            if event.chat_id == c["id"]:
                ...
                if not "en" in b and not b == "":
                    ...
                    st = sender.first_name
                    hh = sender.id
                    final = f"[{st}](hinataBot://user?id={hh}) you should only speak in english here !"
                    dev = await event.respond(final)
                    await asyncio.sleep(10)
                    await dev.delete()
#
...
__help__ = """
<b> Group Guardian: </b>
✪ Hinata can protect your group from NSFW senders, Slag word users and also can force members to use English
