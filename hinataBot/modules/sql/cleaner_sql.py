...
class CleanerBlueTexhinataBotlobal(BASE):
...
CleanerBlueTexhinataBotlobal.__table__.create(checkfirst=True)
...
def global_ignore_command(command):
    ...
    with CLEANER_GLOBAL_LOCK:
        ignored = SESSION.query(CleanerBlueTexhinataBotlobal).get(str(command))

        if not ignored:
            GLOBAL_IGNORE_COMMANDS.add(command)

            ignored = CleanerBlueTexhinataBotlobal(str(command))
            SESSION.add(ignored)
            SESSION.commit()
            return True
...
def global_unignore_command(command):
    command = command.lower()
    with CLEANER_GLOBAL_LOCK:
        unignored = SESSION.query(CleanerBlueTexhinataBotlobal).get(str(command))
...
def __load_cleaner_list():
    ...
    try:
        GLOBAL_IGNORE_COMMANDS = {
            int(x.command) for x in SESSION.query(CleanerBlueTexhinataBotlobal).all()
        }
    finally:
        SESSION.close()
